...
| L8 | `practical_reasoning.critical_question_objections` implements the Atkinson & Bench-Capon CQ5, CQ6, and CQ11 only. Any other critical question raises `NotImplementedError`. | `src/argumentation/frameworks/practical_reasoning.py` |
| L9 | `enforcement.py` is a brute-force minimal-change reference oracle, **not** Baumann-style normal/strong/weak-expansion enforcement. It may add or remove edges and arguments to satisfy the target. | `src/argumentation/dynamics/enforcement.py` |
| L10 | `datalog_grounding` consumes Gunray's conservative DeLP-compatible `inspect_grounding` only. The Diller (2025) Definition 12 NAP analysis is not implemented. | `src/argumentation/structured/aspic/datalog_grounding.py`, `notes/workstream-datalog-grounding-2026-05-01.md` |
| L11 | The probabilistic strategy alias `dfquad` is accepted by `_ALLOWED_STRATEGIES` but raises downstream -- only `dfquad_quad` and `dfquad_baf` are live. | `src/argumentation/probabilistic/probabilistic.py` |
...
|---|---:|---|---|---|
| P-A.1: arbitrary ADF kernel absent | HIGH | `src/argumentation/frameworks/adf.py` | `tests/frameworks/test_adf_acceptance_condition_ast.py` | closed `06bff8a` |
| P-A.2: flat ABA kernel absent | HIGH | `src/argumentation/structured/aba/aba.py` | `tests/structured/aba/test_aba_bondarenko_examples.py` | closed `70bc537` |
| P-A.3: ADF and ABA foundational papers had no executable kernel | HIGH | `src/argumentation/frameworks/adf.py`, `src/argumentation/structured/aba/aba.py` | `tests/test_workstream_o_arg_aba_adf_done.py` | closed `4719640` |
...
| Finding | Severity | Production surface | First failing test | Status |
|---|---:|---|---|---|
| Bug 1: ideal extension must not union multiple maximal admissible candidates | HIGH | `src/argumentation/core/dung.py` | `tests/core/test_dung_ideal_admissibility.py` | closed `6ea2acf`, hardened `38f94bf` |
| Bug 2: ASPIC literal ids must be valid ASP constants | HIGH | `src/argumentation/structured/aspic/aspic_encoding.py` | `tests/structured/aspic/test_aspic_encodings.py::test_ws_o_arg_aspic_encoding_sanitises_literal_ids_for_asp` | closed `60e9f30` |
| Bug 3: duplicate defeasible rule names must fail at encode time | HIGH | `src/argumentation/structured/aspic/aspic_encoding.py` | `tests/structured/aspic/test_aspic_encodings.py::test_ws_o_arg_aspic_encoding_rejects_duplicate_defeasible_rule_names` | closed `60e9f30` |
| Bug 4: AF revision change classification must use extension content | HIGH | `src/argumentation/dynamics/af_revision.py` | `tests/dynamics/test_af_revision.py::test_ws_o_arg_cayrol_2010_decisive_uses_surviving_extension_content` | closed `ac5ec44` |
| Bug 5: `ExtensionRevisionState` ranking must be lazy | HIGH | `src/argumentation/dynamics/af_revision.py` | `tests/dynamics/test_af_revision.py::test_ws_o_arg_extension_revision_state_accepts_lazy_ranking` | closed `ac5ec44` |
| Bug 6: `strictly_weaker(non-empty, empty)` must match ASPIC set lifting | HIGH | `src/argumentation/core/preference.py` | `tests/core/test_preference.py::TestStrictlyWeakerConcrete::test_ws_o_arg_non_empty_set_is_strictly_weaker_than_empty_boundary` | closed `5a48004` |
...
| Bug 8: Monte Carlo confidence z-score must accept continuous values | HIGH | `src/argumentation/probabilistic/probabilistic.py` | `tests/probabilistic/test_probabilistic.py::test_ws_o_arg_z_for_confidence_accepts_continuous_confidence_values` | closed `5a48004` |
...
| Finding | Severity | Production surface | First failing test | Status |
|---|---:|---|---|---|
| Bench-Capon p. 438 argument chains absent | MED | `src/argumentation/frameworks/vaf_completion.py` | `tests/frameworks/test_vaf_completion.py::test_argument_chain_validates_definition_6_3_and_parity` | closed |
| Bench-Capon p. 439 lines of argument absent | MED | `src/argumentation/frameworks/vaf_completion.py` | `tests/frameworks/test_vaf_completion.py::test_line_of_argument_builds_distinct_value_chains_and_stops_on_repeat` | closed |
| Bench-Capon pp. 440-441 Theorem 6.6 / Corollary 6.7 helpers absent | MED | `src/argumentation/frameworks/vaf_completion.py` | `tests/frameworks/test_vaf_completion.py::test_corollary_6_7_two_value_cycle_matches_preferred_extension` | closed |
| Bench-Capon pp. 444-447 fact-as-highest-value and factual uncertainty absent | MED | `src/argumentation/frameworks/vaf_completion.py` | `tests/frameworks/test_vaf_completion.py::test_fact_argument_blocks_ordinary_attack_and_uncertainty_has_multiple_extensions` | closed |
