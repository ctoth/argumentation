...
The paper presents a constraint-programming implementation path for abstract argumentation, including both classical Dung semantics and weighted variants. Its key idea is to model argument inclusion, attacks, defense, maximality/minimality, and tolerated inconsistency as CSP/SCSP constraints, so the same solver architecture can support crisp, fuzzy, probabilistic, and cost-based argumentation. This is relevant for projects that need executable weighted argumentation semantics rather than only abstract complexity results.
