...
def test_same_value_attacks_always_succeed_even_when_audience_orders_other_values() -> (
    None
):
...
def test_generated_vaf_attacks_succeed_exactly_when_target_value_is_not_strictly_preferred(
    ...
    target_strictly_preferred = (
        audience_rank[target_value] < audience_rank[attacker_value]
    )
    expected = (
        frozenset()
        if target_strictly_preferred
        else frozenset({("attacker", "target")})
    )
