...
@given(flat_aba_frameworks())
@settings(deadline=10000, max_examples=40)
def test_grounded_extension_is_subset_of_every_preferred_extension(
    framework: ABAFramework,
) -> None:
    grounded = native_aba.grounded_extension(framework)

    assert all(grounded <= extension for extension in native_aba.preferred_extensions(framework))


@given(flat_aba_frameworks(), st.data())
@settings(deadline=10000, max_examples=40)
def test_preferred_grounded_preprocessing_is_idempotent(
    framework: ABAFramework,
    data: st.DataObject,
) -> None:
    assumptions = tuple(sorted(framework.assumptions, key=repr))
    ...
    once = aba_sat._preferred_required_with_grounded(framework, required)
    twice = aba_sat._preferred_required_with_grounded(framework, once)

    assert once == twice
    assert native_aba.grounded_extension(framework) <= once


@given(flat_aba_frameworks(), st.data())
@settings(deadline=10000, max_examples=40)
def test_preferred_grounded_preprocessed_witness_lifts_to_native_preferred(
    framework: ABAFramework,
    data: st.DataObject,
) -> None:
    assumptions = tuple(sorted(framework.assumptions, key=repr))
    required = data.draw(st.frozensets(st.sampled_from(assumptions), max_size=2))
    preprocessed = aba_sat._preferred_required_with_grounded(framework, required)

    witness = aba_sat.sat_support_extension(
        framework,
        "preferred",
        require_assumptions=preprocessed,
    )
...
@given(flat_aba_frameworks(), st.data())
@settings(deadline=10000, max_examples=40)
def test_preferred_grounded_preprocessing_preserves_query_derivability(
    framework: ABAFramework,
    data: st.DataObject,
) -> None:
    assumptions = tuple(sorted(framework.assumptions, key=repr))
    required = data.draw(st.frozensets(st.sampled_from(assumptions), max_size=2))
    query = data.draw(st.sampled_from(tuple(sorted(framework.language, key=repr))))
    preprocessed = aba_sat._preferred_required_with_grounded(framework, required)
    ...
    grounded_forced = {
        extension
        for extension in native_aba.preferred_extensions(framework)
        if preprocessed <= extension
    }

    assert grounded_forced == original
    assert {
        native_aba.derives(framework, extension, query)
        for extension in grounded_forced
    } == {
        native_aba.derives(framework, extension, query)
