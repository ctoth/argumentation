...
- `src/argumentation/solver.py` — `solve_dung_extensions(framework, *, semantics, backend="labelling" | ICCMAAFBackend(...))` returns `ExtensionSolverSuccess | SolverBackendUnavailable | SolverBackendError`. The native `"labelling"` backend dispatches semantics to `argumentation.dung` functions. The explicit `ICCMAAFBackend` object delegates SE-* extension queries to `solver_adapters.iccma_af`.
...
- `tests/test_solver_availability.py` — native extension solving, deleted-Z3 rejection, explicit ICCMA extension routing, unavailable/error mapping, native acceptance witnesses/counterexamples, and explicit ICCMA acceptance routing.
...
- `6885bbb` / `d9be3fc`: explicit `ICCMAAFBackend` extension routing.
