"""Adapted grounded-semantics edge-tracking tree-decomposition DP.

Exact grounded acceptance for defeat-only probabilistic worlds (where
`attacks == defeats` and there are no support relations), processing a nice
tree decomposition bottom-up while tracking realized edge configurations and
present forgotten arguments. Construction machinery is imported from
