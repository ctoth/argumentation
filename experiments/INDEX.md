...
full experiments**. Probe 7 is preregistered and **not consumed**; its capability
gate precedes any access to the sole permitted development row.
...
| R1-P7 | Direct CaDiCaL 2.2.1 on the unchanged eager-arc one-shot stable CNF | preregistered; capability not established; probe not consumed | `experiments/2026-07-11-iccma2023-probe-7-cadical221-eager-arc.md` | Pin `rel-2.2.1` / `4198d817d0dcde5b1240eefbff70b555b7df2af9`. Capability and semantic validation precede target access. The sole permitted row is `aba_2000_0.3_10_10_0.aba` SE-ST; formula/variable/clause/phase hashes, one solve, eager/no-fallback shape, signed-literal model semantics, independent UNSAT-proof validation, 9 s internal / 10 s outer caps, and 3× interleaved controls/candidates are frozen. A positive diagnostic only authorizes a separately preregistered routing experiment because production currently excludes the 600-assumption row. Budget remains 6/8. |
