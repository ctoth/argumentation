...
def _load(stem: str):
    ...
    query = (
        (ROOT / "extracted" / "instances" / "AFs" / f"{stem}.af.arg")
        .read_text()
        .strip()
    )
...
def _run(framework, query, op, engine):
    ...
    if op == "se_pr":
        prepared = af_sat._prepare(
            framework, "preferred", simplify=True, require_in=None, require_out=None
        )
        if prepared is None:
            return None
        problem = af_sat.AfSatKernel(
            prepared.residual, check_budget_seconds=CHECK_BUDGET_SECONDS, engine=engine
        )
...
def main() -> int:
    ...
    print(
        f"{stem} {op} engine={engine}: {len(framework.arguments)} args / {len(framework.defeats)} defeats",
        flush=True,
    )
    t = perf_counter()
    try:
        ...
        print(
            f"TIMEOUT (check budget {CHECK_BUDGET_SECONDS}s) time={perf_counter() - t:.2f}s",
            flush=True,
        )
    return 0
